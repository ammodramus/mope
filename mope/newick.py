# coding: utf8
"""
This file is a modified version of python-newick:

    https://github.com/glottobank/python-newick

This software is licensed under the Apache 2.0 License. You may obtain a copy
of the License at
...
class Node(object):
    ...
    def __init__(self, name=None, length=None, **kw):
        ...
        if 'num_freqs' in kw and 'num_loci' in kw:
            self.cond_probs = np.ones((kw['num_loci'], kw['num_freqs']))
        look_for_multiplier = kw.pop('look_for_multiplier', False)
        self.multipliername = None
    ...
    def set_multiplier_values(self, ages):
        for node in self.walk('postorder'):
            if node == self:
                continue
            mname = node.multipliername
            if mname:
                node.multipliervalues = ages[mname]
